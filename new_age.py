...
if today_month < dob_month:
    if today_day < dob_day:
        print(f"age is : {difference_age-1} years, {dob_month - today_month} months and {dob_day - today_day} days  ")
    elif today_day == dob_day:
        print(f"age is : {difference_age-1} years and {dob_month - today_month} months")
    else:
        print(f"age is : {difference_age-1} years, {dob_month - today_month-1} months and {dob_day+30 - today_day} days")
    
  #print(f"age is : {difference_age-1}")
elif today_month > dob_month:
    if today_day < dob_day:
        print(f"age is : {difference_age} years, {dob_month+12 - today_month} months and {dob_day - today_day} days  ")
    elif today_day == dob_day:
        print(f"age is : {difference_age} years and {dob_month+12 - today_month} months")
    else:
        print(f"age is : {difference_age} years, {dob_month+12 - today_month-1} months and {dob_day+30 - today_day} days")
...
# if the ongoing month number is same as DOB_month, than the same condition as above will be applied to dates, meaning if todays date is less than dob-date than 1 will be subtracted otherwise it will stay the same
  ...
  if today_month == dob_month:
    if today_day < dob_day:
      print(f"age is : {difference_age-1} years and {dob_day - today_day} days ")
    else:
      print(f"age is : {difference_age} and {dob_day+30 - today_day} days")
